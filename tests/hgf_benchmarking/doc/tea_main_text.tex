...
For example, since many left/right neuronal pairs (which are sisters in the ontology) have almost identical annotations, with at most one or two gene differences between them%. We reasoned that when two tissues have almost identical annotations
, we cannot have statistical confidence in differentiating between them. As a result, testing these sister tissues provides no additional information compared with testing only the parent node to these sisters. %We refer to such sisters as `redundant'. 
...
We have endeavoured to benchmark our tool well. However, our analysis could not identify %identify good negative controls
...
\begin{figure}
  ...
  \captionsetup{width= 0.95\textwidth}
  \caption{\csentence{Independently derived gene sets show similar results 	when tested with the same dictionary.}
...
\begin{figure}[h!]
	...
	\captionsetup{width= 0.95\textwidth}
  	\caption{\csentence{\emph{D.~coniospora} Gene Enrichment Analysis and Tissue Enrichment Analysis results.}
	...
	\textbf{b)} TEA results, sorted by q-value (lowest on top). 
