...
Lnames= ['Otorhabdus luminescens', 'Serratia marcescens', 'Enterococcus faecalis',
         'Harposporium sp', 'Drechmeria coniospora']
...
for i, df in enumerate(Ldf):
    ...
    for direction in Ldirection:
        ...
        df_res, unused= tea.enrichment_analysis(x, tissue_df, show= False, alpha= 0.1)
...
for species in Lreceptacles:
    ...
    i=0
    if n> 1:     
        fig, ax= plt.subplots(nrows= n, figsize= (6, 8))
        fig.subplots_adjust(top=2)
        fig.suptitle(species, fontsize= 15, y=1.02)
        
        
        tea.plot_enrichment_results(current.result_dict['Infection_downregulated'], title= 'Name', save= False, fig= fig, ax= ax[0])
        ...
        tea.plot_enrichment_results(current.result_dict['Infection_upregulated'], title= 'Name', save= False, fig= fig, ax= ax[1])
        ax[1].set_ylabel('Up-Regulated Tissues')
        xlabel= ax[1].set_xlabel('Enrichment Fold Change - {0}'.format(species))
        ax[1].yaxis.set_label_position('right')
        fig.tight_layout()
        plt.savefig('../output/Engelmann/Graphs/'+species+'Enrichment.png', rect=[0, 0.03, 1, 0.95], bbox_extra_artists=[xlabel], bbox_inches='tight')
#        plt.close()
        
    else:
        fig, ax= plt.subplots(nrows= n, figsize= (6, 4))
        fig.subplots_adjust(top= 0.85)
        fig.suptitle(species, fontsize= 15, y= 1.05)


        if 'Infection_downregulated' in current.result_dict:
            ax= tea.plot_enrichment_results(current.result_dict['Infection_downregulated'], title= 'Name', save= False, fig= fig, ax= ax)
            ax.set_ylabel('Down-Regulated Tissues')
        else:
            ax= tea.plot_enrichment_results(current.result_dict['Infection_upregulated'], title= 'Name', save= False, fig= fig, ax= ax)
